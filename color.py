"""Predefined named colors, easy to use and access. These colors are computer-
generated. Towards the end, functions are provided to modify and edit colors,
as well as changing their format and appearance.

A Color class is provided, but is not currently working for anything like
widgets and sprites, etc. You can use its properties like hex and rgb instead.
"""
...
class Color:
    ...
    def __init__(self, color, alpha = 255):
        ...
        if not self.alpha < 255 and self.alpha > 0:
            raise ValueError(
                             "For all colors, the alpha must be a value less "
                             "than 255 and greater than 0."
                            )
    ...
    def scale(self, factor, alpha = None):
        ...
        r, g, b = int(self.hex[:2], 16), int(
            self.hex[2 : 4], 16), int(self.hex[4:], 16)
